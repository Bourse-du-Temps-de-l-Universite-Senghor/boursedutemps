...
const Members: React.FC<MembersProps> = ({ users, onViewProfile }) => {
  ...
  const filtered = users.filter(u => 
    ...
    u.skills.some(s => s.toLowerCase().includes(filter.toLowerCase()))
  );
          ...
          >
            ...
            <div className="flex flex-wrap justify-center gap-1 mb-6">
              {u.skills.slice(0, 3).map((s, i) => (
                <span key={i} className="text-[10px] bg-slate-50 px-2 py-1 rounded-full font-medium text-slate-500">
                  {s}
                </span>
              ))}
            </div>
            <div className="pt-4 border-t border-slate-50">
              <span className="text-xs font-bold text-slate-400">⏰ {u.credits} crédits</span>
            </div>
          </div>
        )) : (
